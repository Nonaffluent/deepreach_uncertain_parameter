...
p.add_argument('--experiment_name', type=str, default='1x1test', required=False,
...
p.add_argument('--velocity', type=float, default=0.1, required=False, help='Speed of the 2D drone')
p.add_argument('--disturbance_mag', type=float, default=0.1, required=False, help='Disturbance Magnitude')
p.add_argument('--theta_range', type=float, default=math.pi/6, required=False, help='Range for possible headings in radians')
p.add_argument('--collisionR', type=float, default=0.2, required=False, help='Collision radius')
p.add_argument('--minWith', type=str, default='none', required=False, choices=['none', 'zero', 'target'], help='BRS vs BRT computation')

p.add_argument('--position_alpha', type=float, default=1.0, required=False, help='Position range for normalization')

p.add_argument('--clip_grad', default=0.0, type=float, help='Clip gradient.')
p.add_argument('--use_lbfgs', default=False, type=bool, help='use L-BFGS.')#particular opt algorithm
...
dataset = dataio.ReachabilityDrone2DSource(numpoints=100000, collisionR=opt.collisionR, velocity=opt.velocity, 
                                          disturbance_mag=opt.disturbance_mag, theta_range=opt.theta_range,
                                          pretrain=opt.pretrain, tMin=opt.tMin,tMax=opt.tMax,
                                          counter_start=opt.counter_start, counter_end=opt.counter_end,
                                          pretrain_iters=opt.pretrain_iters, seed=opt.seed,
                                          position_alpha=opt.position_alpha,
...
# Define the loss
loss_fn = loss_functions.initialize_hji_drone2D(dataset, opt.minWith)
...
def val_fn(model, ckpt_dir, epoch):
  ...
  # Create a figure
  fig = plt.figure(figsize=(5,5*num_times))#figsize (w,l) in inches
  ...
  for i in range(num_times):
    ...
    #mgrid_coords=mgrid_coords/opt.position_alpha
