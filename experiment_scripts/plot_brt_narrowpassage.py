...
# Load the model
model = modules.SingleBVPNet(in_features=11, out_features=1, type='sine', mode='mlp', final_layer_factor=1., hidden_features=512, num_hidden_layers=3)
model.cuda()
root_path = os.path.join('./deepreach_uncertain_parameter/experiment_scripts/logs', 'NarrowPassage1')
...
# Create figures
fig = plt.figure(figsize=(8*num_slices, 3.8*num_times))
fig_valfunc = plt.figure(figsize=(8*num_slices, 3.8*num_times))
...
def add_environment_stuff(ax, R2_pos=None):
  ## Plot obstacles
  # Stranded vehicle
  #obs_stranded = plt.Circle((dataset.stranded_car_pos[0], dataset.stranded_car_pos[1]), 0.5*dataset.L, color='gray', alpha=0.5)
  obs_stranded = patches.Ellipse((dataset.stranded_car_pos[0], dataset.stranded_car_pos[1]), dataset.le, dataset.wi, color='gray', alpha=0.5)
  ax.add_artist(obs_stranded)

  # Outside radius
  #obs_stranded = plt.Circle((dataset.stranded_car_pos[0], dataset.stranded_car_pos[1]), dataset.L, color='gray', alpha=0.5)
  obs_stranded = patches.Ellipse((dataset.stranded_car_pos[0], dataset.stranded_car_pos[1]), dataset.le + dataset.L, dataset.wi + dataset.L, color='gray', alpha=0.5)
  ax.add_artist(obs_stranded)
...
fig.savefig(os.path.join('./deepreach_uncertain_parameter/experiment_scripts/logs', 'NPassage1.png'))
fig_valfunc.savefig(os.path.join('./deepreach_uncertain_parameter/experiment_scripts/logs', 'NPassage1V.png'))
